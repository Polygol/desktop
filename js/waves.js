                    ...
                    try {
                        ...
                        if (videoEl) {
                            ...
                            stream.getVideoTracks()[0].onended = () => {
                                ...
                                if (typeof window.closeWindowEmbed === 'function') {
                                    // Only close if the current app is still the cast app
                                    const active = document.querySelector('.app-window[style*="display: block"]');
                                    if (active && active.dataset.embedUrl && active.dataset.embedUrl.includes('cast.html')) {
                                        window.closeWindowEmbed();
                                    }
                                }
                            };
                            return; // Success
...
async function handleRemoteCommand(payload, peerId) {
    ...
    switch (type) {
        ...
        case 'home':
            if (typeof window.minimizeWindowEmbed === 'function') {
                window.minimizeWindowEmbed();
            }
            break;
        ...
        case 'launchApp':
            if (data.url && typeof window.createWindowEmbed === 'function') {
                window.createWindowEmbed(data.url);
            }
            break;
